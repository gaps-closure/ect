from FlowModel import idToColor, idToMsg
...
componentLvl    = lambda c:       "Component '{}' has level '{}'.".format(c.name, idToColor(c.level))
remoteLvl       = lambda c:       "Component '{}' has remotelevel '{}'.".format(c.name, idToColor(c.remotelevel))
flowMsg         = lambda f:       "Flow '{}' has message '{}'.".format(f.name, idToMsg[f.msg])
flowLabel       = lambda f:       "Flow '{}' has label '{}'.".format(f.name, f.label.name)
labelLocal      = lambda l:       "Label '{}' has level '{}'.".format(l.name, idToColor(l.local))
labelRemote     = lambda l:       "Label '{}' has remotelevel '{}'.".format(l.name, idToColor(l.remote))
...
denyFlow        = lambda m, o, g: "The cross-domain flow of message '{}' from level '{}' to '{}' is disallowed by the provided ruleset".format(idToMsg[m], idToColor(o), idToColor(g))
