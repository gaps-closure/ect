...
{-| Called by the genProveEquiv instances when the match fails, e.g.,
    when passed two dissimilar constructors -}
proveEquivFail :: (Show t) => t -> t -> ProofM Equiv
proveEquivFail a b = proofFail $
  "Cannot compare two different constructors for equivalence: (" ++
