...
#pragma clang attribute push (__attribute__((annotate("PROCESS_SECINPUT"))), apply_to = any(function,type_alias,record,enum,variable(unless(is_parameter)),field))
...
#pragma clang attribute push (__attribute__((annotate("EMBEDDING_SHAREABLE"))), apply_to = any(function,type_alias,record,enum,variable(unless(is_parameter)),field))
...
void run_secdesk_service(int argc, char const *argv[]) {
  ...
  _rpc_start_recognizer();
  fio_start(.threads = fio_cli_get_i("-t"), .workers = fio_cli_get_i("-w"));
  _rpc_stop_recognizer();
  stop_imageprocessor();
  stop_database();
  fio_cli_end(); 
}
